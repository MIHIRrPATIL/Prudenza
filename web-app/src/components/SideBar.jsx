  ...
  return (
    <aside
      className={`border rounded-4xl text-white/70 p-3 border-white/40 
       bg-black transition-[width] duration-300 ${
        expanded ? "" : "w-20"
      }`}
    >
      <nav className="h-full flex flex-col">
        ...
        >
          <div
            className={`p-2 items-center ${expanded ? "flex" : "flex-col gap-3"}`}
          >
            <Image
        ...
        <div
          className={`border-t border-white/20 justify-between items-center py-2 pt-5 ${
            expanded ? "flex gap-2" : "flex-col mx-auto"
          }`}
        >
          ...
          >
            <button className="bg-red-500/70 flex-shrink-0 hover:bg-red-500/90 rounded-xl p-1.5">
              <LogOut size={20} />
            </button>
          </div>
        </div>
